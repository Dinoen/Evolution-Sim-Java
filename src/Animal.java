...
public abstract class Animal extends Living {
        ...
        for (int i = 0; i < Main.allEntities.size(); i++) {
            ...
            for (int j = 0; j < Main.allEntities.get(i).getEntities().size(); j++) {
                ...
                if (Main.allEntities.get(i).getEntities().get(j) != null) {
                    ...
                    float currentDistance = p.dist(this.getLocation().x, this.getLocation().y,
                            potentialTarget.getLocation().x, potentialTarget.getLocation().y);
    ...
    public float reCombinationSpeed(float fatherSpeed, float motherSpeed) {
        ...
        if (ourRandomNumber <= 30) {  //first 45 % chance 0-44, then just return the female original speed
            return fatherSpeed;
        }
        else if (ourRandomNumber >= 31 && ourRandomNumber <= 60) { //next 45% chance of the fathers genes being given to the children
            return motherSpeed;
        }
        else if (ourRandomNumber >= 61 && ourRandomNumber <= 80) { //the last 10 percent from original random
        ...
        else if (ourRandomNumber >= 81) {//Last 5 % of the percent chance from the start of the function
            System.out.println("Mutation has happened");
